...
# New rest state instruction messages
MSG_REST_CLOSED = ['Please close your eyes',
                  'and relax for 5 minutes',
                  'Keep your eyes closed until instructed otherwise']
...
REST_PERIOD = 300000  # 5 minutes (300,000 ms)
...
def main():
    ...
    # First resting state (eyes closed)
    if progress_file:
        update_progress(progress_file, 10, "First resting state (eyes closed) preparing...")
    
    # Display eyes closed instructions
    if display_message(screen, font, MSG_REST_CLOSED, CLC_INSTR):
        return
    
    if progress_file:
        update_progress(progress_file, 15, "First resting state (eyes closed) in progress...")
    
    # Start eyes closed resting period
    send_keystroke()
    # Black screen during eyes closed period
    if display_message(screen, font, "", REST_PERIOD):
        return
    send_keystroke()
    
    if progress_file:
        update_progress(progress_file, 35, "First resting state complete. Starting second resting state...")
    
    # Second resting state (eyes open)
    # Display eyes open instructions
    if display_message(screen, font, MSG_REST_OPEN, CLC_INSTR):
        return
    
    if progress_file:
        update_progress(progress_file, 40, "Second resting state (eyes open) in progress...")
    
    # Start eyes open resting period with fixation cross
    send_keystroke()
    ...
    if progress_file:
        update_progress(progress_file, 65, "Resting states complete. Trials ready.")
